...
var $ = require("gulp-load-plugins")({
    pattern:
        ["gulp-*", "gulp.*", "del*", "path*", "stream-series"], lazy: true
});
...
gulp.task("clean", function (cb) {
    $.del.sync([
      "./debug", "./release", "./app", "./vendor", "./content", "./fonts"
    ], {
        force: true
    });
    cb();
...
gulp.task("vendorScripts", function () {
    return gulp
        ...
        .pipe(gulp.dest($.path.join(paths.dist.release, "vendor")))
});
...
gulp.task("fonts", function () {
    return gulp
        ...
        .pipe($.plumber())
        .pipe($.flatten())
        .pipe(gulp.dest($.path.join("fonts")))
...
gulp.task("release", ["app"], function () {
    var sources = gulp
        .src([
          $.path.join(paths.dist.release, "vendor/**/*.*"),
